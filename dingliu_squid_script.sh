...
#############################################
# Integrated AEAD Proxy 部署管理脚本 v4.0
...
show_main_menu() {
    clear
    echo -e "${CYAN}=========================================="
    echo -e "   Integrated AEAD Proxy 管理中心 v4.0"
...
install_proxy_binary() {
    ...
    # 查找二进制文件
    local found=false
    for path in "./integrated-aead-proxy" "../integrated-aead-proxy" "./target/release/integrated-aead-proxy"; do
        if [ -f "$path" ]; then
    ...
    if [ "$found" = false ]; then
        error "未找到integrated-aead-proxy程序"
        error "请先编译: cargo build --release"
        exit 1
    fi
...
update_proxy() {
    ...
    if [ "$found" = false ]; then
        error "未找到新的integrated-aead-proxy程序"
    fi
}
